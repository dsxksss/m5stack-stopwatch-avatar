...
| 能力 | 当前固件接入 | 验证边界 |
| --- | --- | --- |
| AMOLED 显示 | 11 种程序化表情、局部刷新、全屏长文字打字机、公网 JPEG/PNG 阅读页和诊断界面 | 已编译、烧录并持续显示；公网测试完成文字页到 PNG 页切换，正常表情约 59–60 fps；更多图片尺寸仍需目视回归 |
...
| Wi-Fi | 最多保存 5 个 2.4 GHz 网络，扫描并连接信号最强的可见已保存项；门户可增删改；联网后提供 NTP、本地阅读配置和公网 HTTPS 书源 | 升级后旧网络已恢复并自动重连；HTTPS 证书、清单、文字和 PNG 下载已在真机跑通。多 AP 实际漫游仍需用户验收 |
