...
- 长按 A+B 进入硬件诊断；
- 长按 A 呼出完全由眼睛组成的极简菜单；单击 A 浏览电量、亮度、音量和网络，双击 A 确认，B 返回。电量和网络继续用表情传达状态，亮度与音量直接由眼睛睁开幅度表达；
- 长按 B 只进入以表情为中心的 Wi-Fi 状态模式，不改变当前连接；松开后再次长按 B 约两秒才会明确开启 `KK-XXXX` 临时配网热点。KK 用眼睛表达配网、连接、成功或失败，且新网络成功连接后才会替换设备 NVS 中的旧凭据；
...
| 网络页 | 双击 A 明确开始配网，B 取消或返回；新网络成功前保留旧凭据 |
| 长按 B | 只进入 Wi-Fi 状态模式，不改变当前网络连接 |
| Wi-Fi 模式内 A / B | A 返回当前表情；单击 B 显示配网提示；长按 B 约两秒才开始配网 |
| 长按 A+B | 进入 / 退出硬件诊断 |
