...
| 能力 | 当前固件接入 | 验证边界 |
| --- | --- | --- |
| AMOLED 显示 | 11 种程序化表情、局部刷新、全屏长文字打字机模式和诊断界面 | 已编译、烧录并持续显示；95 字符、8 行、2 页叙事样例约 59.4–59.7 fps |
...
| Wi-Fi | 自动连接已保存的 2.4 GHz 网络；临时 WPA2 热点、DNS 和网页完成配网；联网后提供 NTP 自动校时和专用长文阅读页 | 配网已在真机通过手机门户验证；`GET /read` 与 UTF-8 长文投送已通过局域网实测。旧的通用眼睛消息服务仍保持移除 |
