...
| B 键 | 普通界面不单独切换表情；与 A 同时双击触发生日彩蛋；眼睛菜单内返回；诊断模式重绘 | 已接入；生日彩蛋的实体双键节奏仍需真机人工验收 |
...
| Wi-Fi | 自动连接已保存的 2.4 GHz 网络；临时 WPA2 热点、DNS 和网页完成配网；联网后为 NTP 自动校时提供传输 | 配网已在真机通过手机门户验证；局域网消息服务已从当前固件移除 |
