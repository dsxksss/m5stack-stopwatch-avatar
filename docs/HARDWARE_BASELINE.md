...
| 能力 | 当前固件接入 | 验证边界 |
| --- | --- | --- |
| AMOLED 显示 | 11 种程序化表情、局部刷新和诊断界面 | 已编译、烧录并持续显示；稳定动画窗口约 60 fps |
| BMI270 | 屏幕坐标倾斜、陀螺仪前馈、移动惯性和诊断数据 | 已在真机观察倾斜与惯性反馈；不同握持方式仍需调节 |
| A 键 | 上一个表情；长按打开眼睛菜单；菜单内单击浏览/调档、双击确认；Wi-Fi 快捷模式返回；诊断模式震动测试 | 已接入并用于原型浏览；发布前仍建议回归测试 |
| B 键 | 下一个表情；眼睛菜单内返回；长按查看 Wi-Fi 状态；Wi-Fi 快捷模式内再次长按约两秒确认配网；诊断模式重绘 | 已接入并用于原型浏览；发布前仍建议回归测试 |
...
| Wi-Fi | 自动连接已保存的 2.4 GHz 网络；临时 WPA2 热点、DNS 和网页完成配网 | 已在真机通过手机门户提交凭据并连接 2.4 GHz 路由器；首次验证获得 `192.168.5.57`，RSSI `-51 dBm` |
| RTC | RX8130 当地时间、状态页和夜间困倦行为；支持串口校时 | 已在真机写入并回读时间；自动网络校时尚未接入 |
