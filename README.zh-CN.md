...
- 长按 B 进入以表情为中心的 Wi-Fi 模式；KK 会临时开放 `KK-XXXX` 配网热点，用眼睛表达配网、连接、成功或失败，提交的凭据只保存在设备 NVS；
...
| A / B | 上一个 / 下一个表情 |
| 长按 A | 进入沉浸式状态表情；再次长按 A 调整音量，或长按 B 调整亮度 |
| 状态内三击 A | 显示 / 隐藏右眼电量百分比文字，并保存选择 |
| 长按 B | 进入 Wi-Fi 模式；手机连接 `KK-XXXX`，密码为 `kkfriend`，随后按配网页面操作 |
| Wi-Fi 模式内 A / B | 返回当前表情 / 开始或重新配网 |
| 长按 A+B | 进入 / 退出硬件诊断 |
...
RTC 使用设备当地时间，不自动处理时区。`dim` 和 `screenoff` 的单位为秒；亮度、超时、安静时段和 Wi-Fi 凭据会持久化保存。配网页面仅支持 2.4 GHz 网络，最长开放五分钟，且不会把提交的密码输出到串口。
