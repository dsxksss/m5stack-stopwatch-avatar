...
| A 键 | 普通界面长按打开眼睛菜单，短按无操作；与 B 同时双击触发生日彩蛋；菜单内单击浏览/调档、双击确认；阅读中返回上一页；诊断模式震动测试 | 阅读页返回已在真机从 `9/22` 回到 `8/22`；生日彩蛋的实体双键节奏仍需真机人工验收 |
| B 键 | 普通界面长按打开独立模式菜单、短按无操作；阅读中短按补完/翻页、长按退出；与 A 同时双击触发生日彩蛋；眼睛菜单内返回；诊断模式重绘 | 正向翻页与 `1507 ms` 长按退出已在真机运行；0.12.2 确认退出后不会复用同一次按住事件重新打开菜单 |
